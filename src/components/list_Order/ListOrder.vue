        ...
        <div class="table-responsive">
            <div class="tableFixHead" style="height: 800px; overflow: scroll; position: relative;">
                <table class="table table-striped table-dark text-white table-hover" style="margin-bottom: 0;">
                    <thead>
                        <tr>
                            <th class="headerLstOrder">Customer</th>
                            <th class="headerLstOrder">Phone</th>
                            <th class="headerLstOrder">Address</th>
                            <th class="headerLstOrder">Email</th>
...
import DetailOrder from './DetailOrder.vue';
    export default {
        ...
        computed:{
            listOrder() {
                if(this.toggle==true) {
                    return this.$store.state.listOrder.filter(item => item.status === 'WAITING'&&item.username.toLowerCase().match(this.search.toLowerCase()))
                } else {
                    return this.$store.state.listOrder.filter(item => item.status === 'COMPLETED'&&item.username.toLowerCase().match(this.search.toLowerCase()))
                }
        ...
        methods: {
            confirmOrder(id) {
                this.$store.dispatch('fetchUpdateOrder',{"id": id, "status": "COMPLETED","ownerId": parseInt(localStorage.getItem('ownerId')) });
            },
            waitingOder(){
                this.$store.dispatch('fetchListOrder');
                this.toggle = true
